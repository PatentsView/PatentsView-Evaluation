...
```{python}
summarizers = {filename: InventorDisambiguationSummary(filename) for filename in ['rawinventor.parquet'] }
```
...
```{python}
compare_plots(*[s.plot_cluster_size_distribution() for s in summarizers.values()])
```
...
```{python}
summarizers["rawinventor.parquet"].get_top_inventors(n=20)
```
...
```{python}
compare_plots(*[s.plot_cluster_unique_name_distribution() for s in summarizers.values()])
```
...
disambiguations = {filename: read_disambiguation(filename) for filename in ['rawinventor.parquet'] }
```
...
```{python}
#| column: page
style_and_display(inspect_clusters_to_split(disambiguations["rawinventor.parquet"], load_israeli_inventors_benchmark()), by="prediction")
```
...
```{python}
#| column: page
style_and_display(inspect_clusters_to_split(disambiguations["rawinventor.parquet"], load_patentsview_inventors_benchmark()), by="prediction")
```
...
```{python}
#| column: page
style_and_display(inspect_clusters_to_merge(disambiguations["rawinventor.parquet"], load_israeli_inventors_benchmark()), by="reference")
```
...
```{python}
#| column: page
style_and_display(inspect_clusters_to_merge(disambiguations["rawinventor.parquet"], load_patentsview_inventors_benchmark()), by="reference")
``` 
