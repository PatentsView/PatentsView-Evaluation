...
```{python}
#| label: cluster-size-distribution-plot
...
The above plot provides information about the number of inventors with a given number of (co-)authored patents. 

We can read from the plot the number of inventors with a single (co-)authored patent, with exactly two (co-)authored patents, and so forth. This distribution of the number of (co-)authored patents patents per inventor is equivalently called the **cluster sizes distribution** of the disambiguation.

When comparing disambiguation results, look for major shifts in the cluster size distribution. Is one of the distribution more skewed to the left than another? This could indicate that one of the disambiguation favors smaller clusters, possibly resulting in higher precision but lower recall. 
