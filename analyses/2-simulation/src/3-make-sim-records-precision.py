...
predictions = {rate: pd.read_csv(f"output/prediction_rate_{rate}.tsv", sep="\t").set_index("mention-id")["inventor_id"] for rate in rates}
...
estimators = {
    ...
    "P_single_block": lambda ref, benchmark: pairwise_precision_estimator(ref, benchmark, sampling_type="single_block", weights="uniform"),
    "P_record": lambda ref, benchmark: pairwise_precision_estimator(ref, benchmark, sampling_type="record", weights="uniform"),
    "P_cluster_block": lambda ref, benchmark: pairwise_precision_estimator(ref, benchmark, sampling_type="cluster_block", weights="cluster_size")
}
...
data["estimates"] = data.apply(lambda row: simulation(estimators[row.estimator], row.sample_size, row.rate, repetitions=100), axis=1, result_type="reduce")
